...
func (dq *DelayQueue) ScannDelayBucket(ctx context.Context, name string) error {
	...
	if err != nil {
		log.ErrorfOutStdoutFile("zrange JobID from DelayBucket failed:%s\n", err.Error())
		return err
	}
	if len(result) == 0 {
		log.InfofOutStdoutFile("%s is empty\n", name)
		return nil
	}
	for _, memeber := range result {
		key, ok := memeber.Member.(string)
		if !ok {
			log.ErrorfOutStdoutFile("assertion failed")
			return errors.New("assertion failed")
		}
		job, err := models.GetJob(ctx, dq.Client, key)
		if err != nil {
			log.ErrorfOutStdoutFile("%s is not exist and get failed:%s\n", key, err.Error())
			return err
		}
		if job.State != consts.State_Delay {
			dq.Client.ZRem(ctx, consts.DelayBucket, memeber.Member)
			log.InfofOutStdoutFile("%s is in DelayBucket but state is %d\n", key, job.State)
			continue
			...
			if _, err := pipe.Exec(ctx); err != nil {
				log.ErrorfOutStdoutFile("%s key is reserved state and add to ReservedBucket failed:%s\n", key, err.Error())
				return err
			}
		}
		if job.Tries >= job.MaxTries && job.MaxTries != -1 {
			...
			_, err := pipe.Exec(ctx)
			if err != nil {
				log.ErrorfOutStdoutFile("%s out max count %d and add to FaileBucket %s\n", key, job.MaxTries, err.Error())
				return err
			} else {
				log.InfofOutStdoutFile("%s success add FaileBucket\n", key)
			}
			continue
		}
		pipe := dq.Client.TxPipeline()
		...
		_, err = pipe.Exec(ctx)
		if err != nil {
			log.ErrorfOutStdoutFile("%s failed add to %s queue %s\n", key, job.Topic, err.Error())
			return err
		} else {
			log.InfofOutStdoutFile("%s success add to %s queue\n", key, job.Topic)
		}
	}
	log.InfofOutStdoutFile("this scanning %d task\n", len(result))
	return nil
}
...
func (dq *DelayQueue) ScannReversedBucket(ctx context.Context, name string) error {
	...
	if err != nil {
		log.ErrorfOutStdoutFile("add to ReservedBucket failed:%s\n", err.Error())
		return err
	}
	if len(result) == 0 {
		log.InfofOutStdoutFile("%s is empty\n", name)
	...
	for _, val := range result {
		...
		if job.State != consts.State_Reserved {
			...
			log.InfofOutStdoutFile("%s is in ReservedBucket but state is %d\n", key, job.State)
			continue
		}
		if time.Now().Unix() > job.PopTime+job.TTR {
			log.ErrorfOutStdoutFile("%s is timeout in\n", key)
			pipe := dq.Client.TxPipeline()
			...
			_, err = pipe.Exec(ctx)
			if err != nil {
				log.ErrorfOutStdoutFile("%s failed hset, lrem and zadd:%s\n", key, err.Error())
				return err
			} else {
				log.InfofOutStdoutFile("%s again refresh time and success hset, lrem and zadd\n", key)
			}
		} else {
			log.InfofOutStdoutFile("%s is executing and remaining time %d\n", key, job.PopTime+job.TTR-time.Now().Unix())
		}
	}
	return nil
}
